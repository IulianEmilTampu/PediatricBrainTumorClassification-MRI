...
@hydra.main(version_base=None, config_path="conf", config_name="debug_conf_BraTS")
...
def set_up(cfg: DictConfig) -> None:
    ...
    # Set save path
    aus_name = f'{cfg.model_version}_opt_{cfg.optimizer}_sch_{cfg.scheduler}_lr_{cfg.base_lr}_expdec_{cfg.exponential_decay if cfg.scheduler == "EXPONENTIAL" else None}_{start_time_hour_minutes}'
    cfg.save_path = os.path.join(
        cfg.root_save_path,
        f"TESTs_BraTS_{start_time_year_month_day}",
        aus_name,
    )
...
def run_training(cfg: DictConfig) -> None:
    ...
    trainset = torchvision.datasets.ImageFolder(
        root=os.path.join(cfg.dataset_path, "train"),
    ...
    validationdataset = torchvision.datasets.ImageFolder(
        root=os.path.join(cfg.dataset_path, "val"),
    ...
    testdataset = torchvision.datasets.ImageFolder(
        root=os.path.join(cfg.dataset_path, "test"),
        transform=validation_transform,
    )
    ...
    print(f"Nbr. training files: {len(trainset)}")
    print(f"Nbr. validation file: {len(validationdataset)}")
    print(f"Nbr. test file: {len(testdataset)}")
    ...
    for epoch in range(cfg.max_epochs):  # loop over the dataset multiple times
        ...
        # NOTE make sure not performing overfitting debugging
        with torch.no_grad():
            ...
            # save stats
            validation_loss_history.append(validation_running_loss / len(trainloader))
            validation_running_loss = 0.0
