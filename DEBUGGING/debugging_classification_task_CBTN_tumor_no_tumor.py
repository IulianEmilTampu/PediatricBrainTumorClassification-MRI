...
@hydra.main(
    version_base=None,
    config_path="conf",
    config_name="debug_conf_CBTN_v1_tumor_no_tumor",
)
...
def set_up(cfg: DictConfig) -> None:
    ...
    cfg.save_path = os.path.join(
        cfg.root_save_path,
        f"TESTs_CBTN_tumor_detection_{start_time_year_month_day}",
        aus_name,
    )
...
def run_training(cfg: DictConfig) -> None:
    ...
    if cfg.model_version.lower() == "resnet9":
        net = model_bucket.ResNet9(nbr_classes=len(cfg.classes_to_use))
    elif cfg.model_version.lower() == "sdm4":
        net = model_bucket.SDM4(nbr_classes=len(cfg.classes_to_use))
    elif cfg.model_version.lower() == "bare_bone_cnn":
        net = model_bucket.Net(nbr_classes=len(cfg.classes_to_use))
    elif cfg.model_version.lower() == "bare_bone_cnn_normalized":
        net = model_bucket.Net_normalized(nbr_classes=len(cfg.classes_to_use))
    else:
        raise ValueError(
    ...
    train_loss_history, validation_loss_history = training_utils.training_routine(
