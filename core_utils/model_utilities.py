...
class ResNets(torch.nn.Module):
    def __init__(
        ...
        as_image_encoder_only: bool = False,
    ):
    ...
    def __init__(
        ...
        if use_SimCLR_pretrained_model:
            ...
            if any(
                ...
                self.model = ResNets(
                    ...
                    freeze_percentage=freeze_percentage,
                    as_image_encoder_only=self.use_age,
                )
            elif version == "2dsdm4":
        ...
        if self.use_age:
            ...
            if any(
                ...
                self.fusion_layer = torch.nn.Linear(2048 + 3, self.nbr_classes)
            else:
                self.fusion_layer = torch.nn.Linear(
                    self.nbr_classes * 2, self.nbr_classes
                )
