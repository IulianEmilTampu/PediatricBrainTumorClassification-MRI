...
SAVE_PATH = pathlib.Path(
    os.path.join(os.path.dirname(SUMMARY_FILE_PATH), "Summary_plots_ISBI")
)
...
# ISBI conference: filter to show only T2w and ViT with and without age on ImageNet pretraining
ORIGINAL_DF = ORIGINAL_DF.loc[
    (ORIGINAL_DF.mr_sequence.isin(["T2", "ADC"])) & (ORIGINAL_DF.pretraining == False)
]
...
# ISBI 2024
mr_sequence_order = ["T2", "ADC"]
model_version_order = ["ResNet50", "ViT_b_16"]
pretraining_version_order = ["ImageNet"]
...
for metric, metric_specs in metric_and_text.items():
    ...
    plot_settings = {
        ...
        "y_axis_tick_font_size": 15,
        "x_axis_tick_font_size": 10,
        ...
        "ymax": metric_specs["metric_range"][1] + 0.05,
        "plot_legend": True,
        "axis": None,
        "numeric_value_settings": {"fontsize": 12, "rotation": -0},
    ...
    if SAVE_FIGURE:
        ...
        fig.savefig(
            os.path.join(SAVE_PATH, file_name + ".pdf"), bbox_inches="tight", dpi=200
        )
        fig.savefig(
            os.path.join(SAVE_PATH, file_name + ".png"), bbox_inches="tight", dpi=200
        )
        plt.close(fig)
    else:
        plt.show()
...
for metric, metric_specs in metric_and_text.items():
    ...
    plot_settings = {
        ...
        "hue_variable": "use_age",
        "hue_order": [False, True],
        "figure_title": "Slice-wise performance",
        "show_ensemble": True,
