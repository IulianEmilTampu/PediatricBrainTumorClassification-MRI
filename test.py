...
def get_line_inputs(su_debug_flag=False):
    # --------------------------------------
    # read the input arguments and set the base folder
    # --------------------------------------
    if not su_debug_flag:
        parser = argparse.ArgumentParser(
            description="Run test on cross validation models"
        )
        parser.add_argument(
            "-wf",
            "--WORKING_FOLDER",
        ...
        args_dict = {
            ...
            "CV_MODEL_FOLDER": "/flush/iulta54/Research/P5-MICCAI2023/trained_models_archive/Classification_optm_ADAM_SDM4_TFRdata_True_modality_T2_loss_MCC_and_CCE_Loss_lr_0.0001_batchSize_32_pretrained_False_frozenWeight_True_useAge_True_no_encoder_useGradCAM_False_seed_1111",
    ...
    # -------------------------------------
    args_dict["SAVE_PATH_TEST"] = os.path.join(
        args_dict["CV_MODEL_FOLDER"], "test_summary"
    )

    Path(args_dict["SAVE_PATH_TEST"]).mkdir(parents=True, exist_ok=True)
...
def check_given_folders(args_dict):
    # -------------------------------------
...
def run_testing(args_dict):

    check_given_folders(args_dict)

    if "SAVE_PATH_TEST" not in args_dict.keys():
        args_dict["SAVE_PATH_TEST"] = os.path.join(
            args_dict["CV_MODEL_FOLDER"], "test_summary"
        )
        Path(args_dict["SAVE_PATH_TEST"]).mkdir(parents=True, exist_ok=True)
        ...
        for mv in ["last", "best"]:
            if M[mv]:
                ...
                utilities.plotConfusionMatrix(
                    ...
                    savePath=args_dict["SAVE_PATH_TEST"],
                ...
                for idx, subj_info in enumerate(per_subjects_files.values()):
                    print(
                        f"Working on subject {idx+1}/{len(per_subjects_files)} \r",
                        end="",
                    )
                    # build generator
    ...
    summary_file = os.path.join(
        args_dict["SAVE_PATH_TEST"], f"tabular_test_summary.csv"
    )
    ...
    for mv in ["last", "best"]:
        try:
            ...
            weighted_MAP_pred = []
            weighted_MODE_pred = []
            for idx, (subject_ID, subject_info) in enumerate(
                per_subjects_files.items()
            ):
    ...
    # save all information on file
    summary_file = os.path.join(
        args_dict["SAVE_PATH_TEST"], f"tabular_per_subject_test_summary.csv"
    )
    csv_file = open(summary_file, "w")
    writer = csv.writer(csv_file)
    csv_header = [
...
if __name__ == "__main__":
    print("Running testing on cross validation models")
    args_dict = get_line_inputs(su_debug_flag=False)
    run_testing(args_dict)
